...
from span_extractors.opt import tensorboardX
from span_extractors import models
from span_extractors.entities import Dataset
from span_extractors.evaluator import Evaluator
from span_extractors.input_reader import JsonInputReader, BaseInputReader
from span_extractors.loss import Loss, SpETLoss, SpERTLoss
from span_extractors.sampling import Sampler
...
class BaseFramework:
    """ Framework base class with common methods """
...
class SpERTFramework(BaseFramework):
    ...
    def eval(self, eval_path: str, types_path: str, input_reader_cls: BaseInputReader):
        args = self.args
        dataset_label = 'test'
    ...
    def ready_inference(self, types_path: str, input_reader_cls: BaseInputReader):
        args = self.args
    ...
    def infer(self, model, document: str, types_path: str, input_reader_cls: BaseInputReader):
        dataset_label = "inference_document"

        # read document
        input_reader = input_reader_cls(types_path, self._tokenizer, self._logger)
        input_reader.read({dataset_label: document})

        # do inference
        outputs = self._eval(model, input_reader.get_dataset(dataset_label), input_reader, inference_only=True)
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: JsonInputReader,
        ...
        if inference_only:
            return evaluator._predictions_json
